            ...
            {weeks.map(([wk, w]) => {
              ...
              return (
                <Card key={wk} className="p-4">
                  ...
                  {emp && (
                    <div className="mt-3">
                      <PayslipButton emp={emp} ledger={ledger} settings={settings}
                        label="View / Download Payslip" className="btn-primary w-full text-sm" iconSize={16}
                        payment={{ date: end.toISOString().slice(0, 10), from_date: wk, to_date: end.toISOString().slice(0, 10), gross: w.earned, recovery: w.recovered, net: w.salaryPaid, method: 'UPI' }} />
                    </div>
                  )}
                </Card>
              );
